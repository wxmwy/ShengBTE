...
void init_cuda_np_(int* _rank, int* _nband, int* _nptk, double* rlattvec, double* energy, double* velocity, double_complex* eigenvect, int* _Nlist, int *list, int* _Ntri, double* Phi, double* R_j, double* R_k, int* Index_i, int* Index_j, int* Index_k, int* IJK, int* types, double* masses, int* Ngrid, int* _nelements, int* _natoms)
{
    ...
    HANDLE_ERROR(cudaSetDevice(rank%GPUNUM));
        ...
        printf("Assumed GPU Memory Usage : %lf GB\n", (double)(sizeof(double)*(9+nptk*Nbands*4+33*Ntri+nelements+1+1+1)+sizeof(double_complex)*(nptk*Nbands*Nbands)+sizeof(int)*(natoms+Ntri*3+nptk*3+NList))/(double)1e9);
#endif
