...
program ShengBTE
  ...
  do ll=1,Nlist
     do ii=1,Nbands
        ...
        ssq = 1.0d+0
        do ix = 1, 3, 1
            if (velocity(List(ll),ii,ix).ne.0.0D+0) then
                absxi = abs(velocity(List(ll),ii,ix))
                if (scale.lt.absxi) then
                    ssq = 1.0d+0 + ssq* (scale/absxi)**2
                    scale = absxi
                else
  ...
  do ll=1,nptk
     do ii=1,Nbands
        ...
        ssq = 1.0d+0
        do ix = 1, 3, 1
            if (velocity(ll,ii,ix).ne.0.0D+0) then
                absxi = abs(velocity(ll,ii,ix))
                if (scale.lt.absxi) then
                    ssq = 1.0d+0 + ssq* (scale/absxi)**2
                    scale = absxi
                else
