...
double Vp_plus(int i,int j,int k,int q,int qprime,int qdprime,
...
{
  double Vp_plus_resx, Vp_plus_resy;
  ...
  for(ll=0;ll<Ntri;++ll) {
      ...
      prefactorx = tmp3_r*tmp1_x;
      prefactory = tmp3_r*tmp1_y;
      temp = prefactorx;
      prefactorx = prefactorx*tmp2_x - prefactory*tmp2_y;
      prefactory = temp*tmp2_y + prefactory*tmp2_x;
     ...
     Vp_plus_resx=Vp_plus_resx+(prefactorx*Vp0x-prefactory*Vp0y);
     Vp_plus_resy=Vp_plus_resy+(prefactorx*Vp0y+prefactory*Vp0x);
  }
  return (Vp_plus_resx*Vp_plus_resx+Vp_plus_resy*Vp_plus_resy);
}
...
double Vp_minus(int i,int j,int k,int q,int qprime,int qdprime,
        double realqprime0, double realqprime1, double realqprime2,
        double realqdprime0, double realqdprime1, double realqdprime2,
        double_complex* eigenvect, int Ntri, double *Phi, double* R_j, double* R_k,
        int* Index_i,int* Index_j, int* Index_k,
        double* masses, int* types, int nptk, int Nbands) 
{
  double Vp_minus_resx, Vp_minus_resy;
  ...
  for(ll=0;ll<Ntri;++ll) {
     ...
     Vp_minus_resx=Vp_minus_resx+(prefactorx*Vp0x-prefactory*Vp0y);
     Vp_minus_resy=Vp_minus_resy+(prefactorx*Vp0y+prefactory*Vp0x);
  }
  return (Vp_minus_resx*Vp_minus_resx+Vp_minus_resy*Vp_minus_resy);
}
    ...
    if (omegap!=0 && omegadp!=0) {
        ...
        if(abs(omega+omegap-omegadp)<=(2.*sigma)) {
            ...
            if (!onlyharmonic) { 
                ...
                atomicAdd(Gamma_plus,hbarp*pi/double(4)*WP3*absVp2);
            }
        }
    }
}
        ...
        if(abs(omega-omegap-omegadp)<=(2.*sigma)) {
            ...
            if (!onlyharmonic) { 
                double absVp2=Vp_minus(i,j,k,list[ll]-1,ii,ss,
                                   ...
                                   Index_i,Index_j,Index_k,
                                   masses, types, nptk, Nbands);
        atomicAdd(Gamma_minus,hbarp*pi/double(4)*WP3*absVp2);
...
#endif
       dim3 blk( (Nbands+1-1)/1, (nptk+32-1)/32, (Nbands+1-1)/1 );
       dim3 blksize( 1,32,1 );
#ifdef DEBUG
       printf("^^ begin CUDA KERNEL ^^\n");
#endif
       HANDLE_ERROR(cudaMemcpy(dWP3, WP3_plus, sizeof(double), cudaMemcpyHostToDevice));
...
//printf("wy-test_ss %d\n",ss);
    ...
    if (omegap!=0 && omegadp!=0) {
       ...
       printf("^^ begin CUDA KERNEL ^^\n");
#endif
       HANDLE_ERROR(cudaMemcpy(dWP3, WP3_minus, sizeof(double), cudaMemcpyHostToDevice));
...
#endif
       HANDLE_ERROR(cudaMemcpy(dWP3, WP3_plus, sizeof(double), cudaMemcpyHostToDevice));
       HANDLE_ERROR(cudaMemcpy(dWP3, Gamma_plus, sizeof(double), cudaMemcpyHostToDevice));
       rta_plus_kernel<<<blk,blksize>>>(Nbands, scalebroad, nptk, T, drlattvec, denergy, dvelocity, deigenvect, NList, dlist, Ntri, dPhi, dR_j, dR_k,  dIndex_i, dIndex_j, dIndex_k, dIJK, dGamma,dWP3, dtypes, dmasses, onlyharmonic, Ngrid[0],Ngrid[1], Ngrid[2],q[0],q[1],q[2], omega, i, ll, d_debug);
#ifdef DEBUG
       printf("^^ begin CUDA KERNEL SYNC ^^\n");
#endif
...
#endif
       HANDLE_ERROR(cudaMemcpy(dWP3, WP3_minus, sizeof(double), cudaMemcpyHostToDevice));
       HANDLE_ERROR(cudaMemcpy(dWP3, Gamma_minus, sizeof(double), cudaMemcpyHostToDevice));
