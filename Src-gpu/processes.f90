...
contains
  ...
  function Vp_plus(i,j,k,q,qprime,qdprime,realqprime,realqdprime,eigenvect,&
    ...
    do ll=1,Ntri
       !prefactor=R_j(1,ll) + R_j(2,ll) + R_j(3,ll)
       !prefactor=dot_product(realqprime,R_j(:,ll))
       !prefactor=realqprime(1)*R_j(1,ll)
       !prefactor=R_J(1,ll)
  ...
  subroutine Ind_driver(energy,velocity,eigenvect,Nlist,List,IJK,N_plus,N_minus,&
    ...
    !write(*,*) Ntri, nptk, Nbands, Nlist, myid, numprocs 

    call init_cuda_Ind_wrapper(rank, Nbands, nptk, rlattvec,maxsize,energy,velocity,eigenvect,Nlist,List,&
               Ntri,Phi,R_j,R_k,Index_i,Index_j,Index_k,IJK,&
               types, masses, Ngrid, nelements, natoms)
    do mm=myid+1,Nbands*NList,numprocs
       ...
       if(N_plus(mm).ne.0) then
          ...
          call run_cuda_Ind_plus_wrapper(rank, mm, Nbands, scalebroad, nptk, T, rlattvec, N_plus(mm), energy,velocity,eigenvect,Nlist,List,&
               Ntri,Phi,R_j,R_k,Index_i,Index_j,Index_k,IJK,&
               Indof2ndPhonon(1:N_plus(mm)),Indof3rdPhonon(1:N_plus(mm)),&
...
!write(*,*) "wy-test-WP3", mm,WP3_plus_reduce(mm)
       ...
       if(N_minus(mm).ne.0) then
          ...
          call run_cuda_Ind_minus_wrapper(rank, mm, Nbands, scalebroad, nptk, T, rlattvec, N_minus(mm), energy,velocity,eigenvect,Nlist,List,&
               Ntri,Phi,R_j,R_k,Index_i,Index_j,Index_k,IJK,&
               Indof2ndPhonon(1:N_minus(mm)),Indof3rdPhonon(1:N_minus(mm)),&
...
!write(*,*) "wy-test-WP3", mm,WP3_minus_reduce(mm)
    ...
    call finalize_cuda_Ind_wrapper()
    deallocate(Gamma0)
       ...
       do j=1,Nbands
          do ii=1,nptk
             ...
             do k=1,Nbands
                ...
                ss=Index_N(qdprime(1),qdprime(2),qdprime(3))
                omegadp=energy(ss,k)
                !write(*,*) j,ii,k,ss, omega, omegap, omegadp
                if ((omegap.ne.0).and.(omegadp.ne.0)) then
                   sigma=scalebroad*base_sigma(&
                        velocity(ii,j,:)-&
                        velocity(ss,k,:))
                   !write(*,*) j,ii,k,sigma, omega, omegap, omegadp
                   if(abs(omega+omegap-omegadp).le.(2.d0*sigma)) then
                      fBEdprime=1.d0/(exp(hbar*omegadp/Kb/T)-1.D0)
                      WP3=(fBEprime-fBEdprime)*&
                           exp(-(omega+omegap-omegadp)**2/(sigma**2))/sigma/sqrt(Pi)/&
                      ...
                      !Gamma_plus=Gamma_plus+(REAL(Vp)+AIMAG(Vp))
                      !Gamma_plus=Gamma_plus+(AIMAG(Vp))
                      !Gamma_plus=Gamma_plus+(REAL(Vp))
                      endif
                   end if
                end if
             end do ! k
